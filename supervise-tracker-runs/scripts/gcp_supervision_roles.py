...
def role_prompt(config, name):
    ...
    cli = shlex.join([sys.executable, config["native_cli"], "--config", config.get("config_path", str(Path(config["state_root"])/"config.json"))])
    replacements = {
    ...
    # The maintained Luna command supplies its own interpreter; other role
    # references use LOG_HELPER as a complete command.
    prompt = prompt.replace("python3 <LOG_HELPER>", cli+" helper --")
...
All coordination and scheduling runs on the target host. Use these exact local transport
...
{cli} read --thread <exact-bound-task-id>
...
The target's directly authorized mission is:
{config.get('mission_context', 'Follow the exact bound mission source in helper status; do not infer completion from idleness.')}

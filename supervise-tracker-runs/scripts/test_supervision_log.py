...
class ImplementationRangeControlTests(unittest.TestCase):
    ...
    def retain_legacy_app_readback_authority(
        ...
        *,
        ...
        policy = supervision_log.read_json(directory / "policy.json")
        current_events = supervision_log.events(directory / "events.jsonl")
        authority_event = f"EVT-{len(current_events) + 1:06d}"
        supervision_log.append_raw(
            directory / "events.jsonl",
            {
                "schema_version": 1,
                "record_id": authority_event,
                "timestamp": supervision_log.utc_now(),
                "target_thread_id": self.target,
                "kind": supervision_log.DIRECT_AUTHORITY_EVENT_KIND,
                "source_class": "direct-user",
    ...
    def test_legacy_app_readback_receipt_rejects_rehashed_attacks(self) -> None:
        ...
        attacks = {
            ...
            "wrong-mission": lambda policy, events: policy[
                "mission_binding"
            ].__setitem__("mission_root", "f" * 64),
    ...
    def test_reviewed_227_byte_local_source_binds_current_started_dag(self) -> None:
        ...
        source_item = "local-reviewed-item-227"; source = f"direct-user:{self.target}:{source_item}"
        source_sha = hashlib.sha256(request.encode()).hexdigest()
        outcome, lifecycle, pending, _ = self.complete_predecessor_and_start_successor(retain_range_authority=False)
        source_turn = "local-reviewed-turn-227"; evidence = [f"source-kind:{supervision_log.DIRECT_AUTHORITY_SOURCE_KIND}", f"source-task:{self.target}", f"source-turn:{source_turn}", f"source-item:{source_item}", f"source-record:{source}", f"source-byte-count:{len(request.encode())}", f"source-sha256:{source_sha}", f"verifier:{self.reviewer}", f"classification:{supervision_log.DIRECT_AUTHORITY_CLASSIFICATION}", "review-findings:none", f"mission-root:{supervision_log.bound_mission(supervision_log.read_json(self.root / self.target / 'policy.json'))['mission_root']}"]; self.call("record", "--target-thread", self.target, "--kind", "meta-review", "--active-block", "0", "--checkpoint", "direct-authority-source-review", "--status", "accepted", "--severity", "info", "--summary", "clean exact source review", "--category", supervision_log.DIRECT_AUTHORITY_REVIEW_CATEGORY, "--model", "gpt-5.6-sol", "--reasoning", "max", "--resolution-owner", "supervisor", "--user-action-required", "no", *(item for value in evidence for item in ("--evidence", value))); review_record = supervision_log.events(self.root / self.target / "events.jsonl")[-1]["record_id"]; review = self.call("implementation-range-authority-source-review-sign", "--target-thread", self.target, "--source-task", self.target, "--source-turn", source_turn, "--source-item", source_item, "--source-record", source, "--source-text-base64", base64.b64encode(request.encode()).decode(), "--review-evidence-record", review_record, "--expected-policy-sha256", supervision_log.read_json(self.root / self.target / "policy.json")["policy_sha256"])["output_json"]
        policy = supervision_log.read_json(self.root / self.target / "policy.json")
        ingested = self.call("implementation-range-authority-source-ingest", "--target-thread", self.target, "--source-task", self.target, "--source-item", source_item, "--source-record", source, "--source-text-base64", base64.b64encode(request.encode()).decode(), "--provenance-review-record", review, "--expected-policy-sha256", policy["policy_sha256"])
        self.call("implementation-range-authority-receipt", "--target-thread", self.target, "--authority-event-record", ingested["record"]["record_id"])
...
class ExecutionEconomyPolicyTests(unittest.TestCase):
    ...
    def test_status_broadcast_policy_validation_accepts_scalar_legacy_predecessor(self) -> None:
        policy = supervision_log.default_policy(self.init_args())
        policy["cross_thread_routing"] = supervision_log.legacy_single_role_cross_thread_routing_contract()
        policy["policy_sha256"] = supervision_log.digest(supervision_log.policy_material(policy))
        self.assertNotIn("status-broadcast", policy["cross_thread_routing"]["purpose_roles"])
        supervision_log.validate_policy(policy)
...
class CrossThreadRoutingGateTests(unittest.TestCase):
    ...
    def route(
        ...
        **route_fields: object,
        ...
        with (
            ...
            mock.patch.object(supervision_log, "events", return_value=route_fields.pop("all_events", [])),
            redirect_stdout(output),
        ):
    ...
    def status_broadcast(self, **changes: object) -> dict[str, object]:
        values = dict(policy=self.policy(), recipient="target-1234", purpose="status-broadcast", action="Utils completed with the accepted revision.",
            source_record="msg_direct_user_1234", source_task="source-task-1234", authority_source_class="direct-user",
            authority_source_record="msg_direct_user_1234", broadcast_scope="utils-completion")
        values.update(changes); return self.route(**values)  # type: ignore[arg-type]
    ...
    def test_status_broadcast_accepts_exact_target_direct_source_and_scope(self) -> None:
        result = self.status_broadcast(); self.assertTrue(result["send_allowed"])
        self.assertEqual((result["recipient_role"], result["status_broadcast"]["source_task_id"]), ("target", "source-task-1234")); self.assertEqual(result["status_broadcast"]["authority_source_class"], "direct-user")
        self.assertEqual(result["status_broadcast"]["authority_source_record"], "msg_direct_user_1234"); self.assertRegex(result["status_broadcast_dedup_key"], r"^status-broadcast:[0-9a-f]{64}$")

    def test_status_broadcast_rejects_missing_mismatched_or_unrelated_provenance(self) -> None:
        cases = ({"source_task": None}, {"authority_source_class": "tracker"}, {"authority_source_record": "msg_other_1234"},
            {"broadcast_scope": ""}, {"recipient": "base-1234"}, {"recipient": "side-thread-1234"})
        for changes in cases:
            with self.subTest(changes=changes), self.assertRaises(supervision_log.SupervisionLogError): self.status_broadcast(**changes)

    def test_status_broadcast_deduplicates_only_the_exact_sent_envelope(self) -> None:
        first = self.status_broadcast(); receipt = {"kind": "notification", "category": "status-broadcast", "status": "sent", "dedup_key": first["status_broadcast_dedup_key"]}
        duplicate = self.status_broadcast(all_events=[receipt])
        self.assertEqual((duplicate["send_allowed"], duplicate["duplicate"]), (False, True))
        changed_policy = self.policy(); changed_policy["updated_at"] = "2026-08-24T07:00:00+00:00"
        changed_policy["policy_sha256"] = supervision_log.digest(supervision_log.policy_material(changed_policy))
        policy_retry = self.status_broadcast(all_events=[receipt], policy=changed_policy)
        self.assertEqual((policy_retry["send_allowed"], policy_retry["duplicate"]), (False, True))
        other_target = copy.deepcopy(changed_policy); other_target["target_thread_id"] = "target-5678"
        other_target["policy_sha256"] = supervision_log.digest(supervision_log.policy_material(other_target))
        for changes in (
            ...
            {"broadcast_scope": "utils-release"}, {"source_task": "source-task-5678"},
            {"policy": other_target, "recipient": "target-5678"},
        ):
            with self.subTest(changes=changes): self.assertTrue(self.status_broadcast(all_events=[receipt], **changes)["send_allowed"])

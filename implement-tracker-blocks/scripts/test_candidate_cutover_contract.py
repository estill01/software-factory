...
class CandidateCutoverContractTests(unittest.TestCase):
    ...
    def _init_supervision(self, source_text: str = "Implement Block 9.") -> None:
        module = cutover._supervision_module()
        source_sha = hashlib.sha256(source_text.encode()).hexdigest()
        mission_sha = hashlib.sha256(b"Candidate cutover mission.").hexdigest()
        ...
        module.append_raw(
            directory / "events.jsonl",
            {
                ...
                "target_thread_id": self.owner_id,
                "kind": module.DIRECT_AUTHORITY_EVENT_KIND,
                "source_class": "direct-user",
                "source_record": "direct-block9-target",
                "source_sha256": source_sha,
                "source_task_id": self.owner_id,
                "source_item_id": "direct-block9-target",
                "verifier_id": "base-reviewer-block9-target",
                "provenance_status": "verified-before-entry",
                "policy_sha256": policy["policy_sha256"],
                "evidence": ["candidate-cutover-test-authority"],
            },
        )
        receipt = module.parser().parse_args(
            [
                "--root",
                str(self.supervision_root),
        ...
        with redirect_stdout(io.StringIO()):
            module.cmd_implementation_authority_receipt(receipt)

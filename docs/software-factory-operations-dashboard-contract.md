...
| Resume supervision | `unavailable` | Block 23 now has a candidate maintained `resume-gate`/`resume-finalize` owner, but it is not a supported dashboard dependency until independent exact-revision acceptance. | Exact Block 23 owner acceptance changes this row to `supported`; Block 24 remains closed until then. |
...
| Resume supervision | Candidate helper lifecycle owner, not yet accepted | Disabled; explain pending exact owner acceptance | Candidate postcondition is one helper-verified `supervision-resume` lifecycle record bound to the exact pause, mission/policy/source fingerprint, and every active owner-derived automation configuration. | Keep disabled until tracker Block 23 accepts the maintained owner; automation or task resume alone remains insufficient. |

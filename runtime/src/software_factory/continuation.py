    ...
    def next_action(self, mission_id: str) -> dict[str, Any]:
            ...
            expired = self.store.all(
                """SELECT l.*,e.work_item_id FROM leases l
                   LEFT JOIN executions e ON e.id=l.owner_execution_id
            ...
            terminal = self.store.all(
                """SELECT id FROM executions WHERE mission_id=?
                   AND execution_type='terminal_verification'
                   ORDER BY created_at DESC LIMIT 1""",
                (mission_id,),
            )
            if not terminal or terminal[0].get("status") != "succeeded":
                return {
                    "posture": "terminal_verification",
                    "action": "run_terminal_verification",
                }
            return {"posture": "terminal_verification", "action": "complete_mission"}
    ...
    def complete_mission(
            ...
            *,
            ...
            with self.store.transaction() as db:
                ...
                if gaps or open_obligations or terminal is None:
                    raise InvalidTransition(
                        "mission cannot complete with capability gaps, open obligations, "
                        "or no successful terminal verification"
                    )
                new_version = expected_version + 1
                now = utc_now()
                db.execute(
